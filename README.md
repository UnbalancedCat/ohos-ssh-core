...
`entry` 模块包含了一个完整的测试 UI，采用状态驱动的折叠式布局，适配手机等小屏设备，可直接在鸿蒙模拟器或真机上运行。

- **入口文件**：[`entry/src/main/ets/pages/Index.ets`](./entry/src/main/ets/pages/Index.ets)
- **UI 特性**：
  - 连接设置区在连接成功后自动折叠，日志区占满中间空间。
  - 底部 Tab 栏切换 Shell / SFTP 模式（两者互斥），控件随模式展开。
  - 适配系统深色/浅色模式，所有颜色与尺寸集中在 `resources` 中管理。
- **演示功能**：
  - 密码认证与私钥认证连接，Host Key SHA-256 指纹展示。
  - **Shell**：执行单次命令 / 启动交互式 PTY / 停止 Shell。
  - **SFTP**：路径导航、目录列出、文本文件读取、二进制流式读取与 ELF 魔数验证。
